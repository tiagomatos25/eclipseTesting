 ...
 *
 */
public class NewClientBoard extends JComponent implements KeyListener {
	private ArrayList<InfoToSend> info;
	...
	public NewClientBoard(ArrayList<InfoToSend> info) {
		this.info = info;
		setFocusable(true);
		addKeyListener(this);
	}
	...
	protected void paintComponent(Graphics g) {
		...
		for (int x = 0; x < Game.DIMX; x++) 
			for (int y = 0; y < Game.DIMY; y++) {
				...
				InfoToSend player = info.get(x*Game.DIMY + y);
				if(player!=null) {
					...
					g.setColor(new Color(player.getId() * 1000));
	...
	public void update(ArrayList<InfoToSend> info) {
		this.info = info;
		this.repaint();
